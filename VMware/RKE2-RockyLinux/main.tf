...
# Rancher is also installed if enabled.                  #
#                                                        #
...
resource "vsphere_virtual_machine" "cluster_node" {
  ...
  clone {
    ...
    # RancherOS2 cloud-init only deploys on inital ISO install, and will not work for templates/clones
    # customize {
    #   linux_options {
  ...
  # The following is for Static IP assignment -- We will default to using DHCP and ignore this for now << FOR REFERENCE ONLY >>
  extra_config = {
  ...
  # Do customizations:  Set hostname, configure RKE2 server nodes
